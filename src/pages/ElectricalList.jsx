...
const ElectricalList = () => {
  ...
  const fetchCatalogItems = async () => {
    try {
      setLoading(true);
      const res = await api.get('/items/?item_type=electrical');
      setItems(res.data);
    } catch (err) {
      console.error('Failed to fetch electrical items:', err);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="glass-card" style={{ 
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.75rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            ...
            <div>
              <span className="badge badge-elec" style={{ marginBottom: '0.15rem' }}>
                ELECTRICAL TRADE
              </span>
              <h1 style={{ fontSize: '1.4rem', fontWeight: 800, color: '#f8fafc', margin: 0 }}>
                {editingId ? 'Edit Electrical Material List' : 'Create Electrical Material List'}
              </h1>
              <p style={{ fontSize: '0.75rem', color: '#94a3b8', margin: 0 }}>
                Wires, switches, MCBs, DB boxes, conduits & accessories
              </p>
            </div>
          </div>

          <Link to="/plumbing-list" className="btn btn-sm btn-outline" style={{ color: '#2dd4bf', borderColor: 'rgba(20, 184, 166, 0.4)' }}>
            <Wrench size={14} /> Switch to Plumbing List <ArrowRight size={14} />
          </Link>
        </div>
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', minWidth: 0 }}>
          ...
          <div className="glass-card" style={{ padding: '1.15rem' }}>
            ...
            <div className="form-group" style={{ marginBottom: 0 }}>
              <label className="form-label">Electrical Notes & Instructions</label>
              <textarea
                name="notes"
                rows="2"
                value={clientInfo.notes}
                onChange={handleClientChange}
                placeholder="e.g. Deliver materials by 10 AM. Use ISI brand FRLS wire & Type-C MCBs."
                className="form-textarea"
              />
            </div>
          </div>

          {/* STEP 2: Electrical Materials Catalog */}
          <div className="glass-card" style={{ padding: '1.15rem' }}>
            <div className="catalog-header-wrap" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.75rem', marginBottom: '0.85rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                <Search size={17} color="#fbbf24" />
                <h3 style={{ fontSize: '1rem', fontWeight: 800, color: '#f8fafc', margin: 0 }}>
                  STEP 2: Select Electrical Materials ({filteredItems.length})
                </h3>
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap', width: '100%', maxWidth: '420px', justifyContent: 'flex-end' }}>
                <div className="catalog-search-wrap" style={{ position: 'relative', flex: 1, minWidth: '180px' }}>
                  ...
                  <input
                    ...
                    placeholder="Search wire, switch, MCB..."
                    className="form-input"
            ...
            <div className="category-scroll" style={{ marginBottom: '1rem', paddingBottom: '4px' }}>
              {categories.map((cat) => (
                <button
                  ...
                  className={`btn btn-sm ${selectedCategory === cat ? 'btn-elec' : 'btn-outline'}`}
            ...
            {loading ? (
              <p style={{ textTransform: 'none', textAlign: 'center', padding: '3rem', color: '#94a3b8' }}>
                Loading electrical material database...
              </p>
            ) : filteredItems.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '2.5rem 1rem', color: '#64748b' }}>
                <p style={{ fontWeight: 600, color: '#94a3b8' }}>
                  {searchQuery ? `No electrical materials match "${searchQuery}"` : 'No electrical materials in this category'}
                </p>
                ...
                >
                  <Plus size={16} strokeWidth={2.5} />
                  <span>Add "{searchQuery || 'Custom Electrical Item'}" as Item</span>
                </button>
              </div>
            ) : (
      ...
      <div className="mobile-fab-drawer">
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <ShoppingBag size={20} />
            <span style={{ fontWeight: 800, fontSize: '0.9rem' }}>View Electrical List</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
